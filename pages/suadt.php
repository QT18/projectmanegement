...
<?php 
    if (isset($_POST['sbm'])) {
        ...
        $sql = "UPDATE detai SET tendt='$tendt', mota='$mota', maloaida='$loaida', magv='$gv', soluong='$sl', ghichu='$gc', trangthai='$tt' WHERE madt='$madt'";
    ...
    if (isset($_GET['id'])) {
        $madt = $_GET['id'];
        $sql_detai = "SELECT * FROM detai WHERE madt=$madt";
        $query_detai = mysqli_query($connect, $sql_detai);
        $row_detai = mysqli_fetch_assoc($query_detai);
    }
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            ...
            <li class="breadcrumb-item active">Sửa đề tài</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="col-md-12">
          <div class="card card-primary">            
            ...
            <div class="form-group">
                <label for="inputMadt">Tên đề tài:</label>
                <input type="text" name="inputMadt" class="form-control" value="<?php echo $row_detai['madt']; ?>" readonly>
              </div>    
              <div class="form-group">
                <label for="inputTendt">Tên đề tài:</label>
                <input type="text" name="inputTendt" class="form-control" value="<?php echo $row_detai['tendt']; ?>">
              </div>
              <div class="form-group">
                <label for="inputMT">Mô tả: </label>
                <textarea name="inputMT" class="form-control" rows="5"><?php echo $row_detai['mota']; ?></textarea>
              </div>
              <div class="form-group">
                ...
                <?php
                    while ($row_loaida=mysqli_fetch_assoc($query_loaida)) {
                        $slt = $row_loaida['maloai'] == $row_detai['maloaida'] ? true : false;
                        ?>
                        <option value="<?php echo $row_loaida['maloai']; ?>" <?php if ($slt) echo "selected"; ?>>
                            <?php echo $row_loaida['tenloai']; ?>
                        </option>
                        <?php
                    }
                ?>
                  ...
                  if(isset($_SESSION['vaitro'])) {
                    ...
                    <?php
                        while ($row_gv=mysqli_fetch_assoc($query_gv)) {
                            ?>
                            <option value="<?php echo $row_gv['id'] ?>">
                              <?php echo $row_gv['ho'].' '.$row_gv['ten']; ?>
                            </option>
                            <?php
                        }
                    ?>
              ...
              <div class="form-group">
                ...
                <input type="number" name="inputSL" class="form-control" min="1" value="<?php echo $row_detai['soluong']; ?>">
              </div>
              
              <div class="form-group">
                <label for="inputGC">Ghi chú:</label>
                <input type="text" name="inputGC" class="form-control" value="<?php echo $row_detai['ghichu']; ?>">
              </div>

              <div class="form-group">
                <label for="inputTrangThai">Trạng thái: </label>
                <select name="inputTrangThai" class="form-control custom-select">
                  <option value="0" <?php if($row_detai['trangthai']==0) echo "selected"; ?>>Đóng</option>
                  <option value="1" <?php if($row_detai['trangthai']==1) echo "selected"; ?>>Mở</option>
                </select>
              </div>
