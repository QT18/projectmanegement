...
<?php require_once __DIR__. "/../config/db.php"; 
    ...
    if(isset($_GET['madk'])&&isset($_GET['stat'])&&isset($_GET['hp'])) {
        $madk = $_GET['madk'];
        $stat = $_GET['stat'];
        $mahp = $_GET['hp'];

        //Cập nhật trạng thái của đăng ký
        $sql = "UPDATE dangky SET trangthai='$stat' WHERE madk='$madk'";
        $query = mysqli_query($connect, $sql);

        if($stat==1) {                
                $sql = "UPDATE dangky SET trangthai='2' ".
                        "WHERE madt=(SELECT madt FROM dangky WHERE madk='$madk') ".
                        "AND madk in (SELECT DISTINCT a.madk FROM nhom a, svhp b WHERE a.masvhp=b.masvhp AND mahp='$mahp')  ".
                        "AND madk <> '$madk'";
                $query = mysqli_query($connect, $sql);

                $sql = "UPDATE dangky SET trangthai='2' ".
                        "WHERE madk in (SELECT DISTINCT a.madk FROM nhom a, svhp b WHERE a.masvhp=b.masvhp AND mahp='$mahp' AND b.masv IN (SELECT masv FROM svhp, nhom WHERE svhp.masvhp=nhom.masvhp AND madk='$madk')) ".
                        "AND madk <> '$madk'";
                $query = mysqli_query($connect, $sql);
        } 

        header('location:gvduyetdk.php');
    }
                    ...
                    <tr>                    
                      ...
                      <?php
                        if ($row['trangthai']==0) {
                          ...
                          <a class="btn btn-danger btn-sm" href="gvduyetdk.php?madk=<?php echo $row['madk']; ?>&stat=2&hp=<?php echo $row['madk']; ?>">
